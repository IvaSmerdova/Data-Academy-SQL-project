...
FROM t_iva_smerdova_project_sql_primary_final
...
CREATE OR REPLACE VIEW v_iva_smerdova_project_avg_wages_per_year AS 
SELECT industry, payroll_year, round(avg(average_wages), 2) AS average_wages_per_year
FROM t_iva_smerdova_project_sql_primary_final
GROUP BY industry, payroll_year
ORDER BY industry, average_wages_per_year;

SELECT * FROM v_iva_smerdova_project_avg_wages_per_year; -- průměrné mzdy za rok

CREATE OR REPLACE VIEW v_iva_smerdova_project_avg_wages_growth AS 
...
FROM v_iva_smerdova_project_avg_wages_per_year vw
JOIN v_iva_smerdova_project_avg_wages_per_year vw2
	ON vw.industry = vw2.industry 
	AND vw.payroll_year = vw2.payroll_year + 1
ORDER BY industry, payroll_year; 

SELECT * FROM v_iva_smerdova_project_avg_wages_growth; -- růst mezd
...
CREATE OR REPLACE VIEW v_iva_smerdova_project_avg_prices_per_year AS 
	SELECT DISTINCT food_category, year(date_from) AS year, round(avg(price), 2) AS avg_price
	FROM t_iva_smerdova_project_sql_primary_final 
	GROUP BY food_category, year(date_from);

SELECT *
FROM v_iva_smerdova_project_avg_prices_per_year; -- prům. cena za rok


CREATE OR REPLACE VIEW v_iva_smerdova_project_avg_price_growth AS 
...
FROM v_iva_smerdova_project_avg_prices_per_year v
JOIN v_iva_smerdova_project_avg_prices_per_year v2
	ON v.food_category = v2.food_category 
	AND v.year = v2.year + 1
ORDER BY food_category, YEAR; -- prům. růst cen potravin

SELECT * 
FROM v_iva_smerdova_project_avg_price_growth 
...
FROM v_iva_smerdova_project_avg_wages_growth vwg
JOIN  v_iva_smerdova_project_avg_price_growth vpg
...
SELECT * FROM t_iva_smerdova_project_sql_secondary_final; 

SELECT *
FROM t_iva_smerdova_project_sql_secondary_final 
WHERE country = 'Czech republic';

CREATE OR REPLACE VIEW v_iva_smerdova_project_gdp_growth as
SELECT t.country, t.year, t2.year + 1 AS next_year,
	round( ( t.gdp  - t2.gdp  ) / t2.gdp  * 100, 2 ) as gdp_growth
FROM t_iva_smerdova_project_sql_secondary_final  t
JOIN t_iva_smerdova_project_sql_secondary_final t2
	ON t.country = t2.country 
	AND t.year = t2.year + 1
WHERE t.country = 'Czech Republic'
ORDER BY t.`year`;

SELECT * FROM v_iva_smerdova_project_gdp_growth; -- růst HDP
...
JOIN  v_iva_smerdova_project_avg_price_growth vpg
	ON vwg.payroll_year = vpg.year
JOIN v_iva_smerdova_project_gdp_growth vgg 
